...
## 2026-07-16 — iFighter 参考飞机 AI 生成素材包（v4 最终）

### 工作内容

从用户提供的 12 张 iFighter 系列游戏截图中识别出 10 种独特飞机类型，跨截图交叉对比确认同一飞机。按 iFighter 的 2.5D 渲染街机风格，使用 AI 图像生成统一风格的参考精灵图。

**v4 修正**（针对用户反馈的两个核心问题）：
1. **roll 姿态**：机头/机身严格保持竖直朝上不旋转，仅机翼一侧抬升另一侧下沉（通过翼面明暗/透视变化表现 roll）
2. **螺旋桨视角**：使用 3/4 俯视角度（iFighter 经典视角），螺旋桨显示为**侧面薄边缘/模糊线条**，非正面圆盘

### 姿态定义

| 姿态 | 后缀 | 描述 |
|------|------|------|
| 直飞 | `_straight` | 机头朝上，机翼水平，螺旋桨侧面 |
| 左横滚 | `_roll_left` | 机头朝上不旋转，右翼高左翼低 |
| 右横滚 | `_roll_right` | 机头朝上不旋转，左翼高右翼低 |

### 交付物

`assets/bak/` 目录，23 个 PNG（RGBA，白底已去）：

7 种飞机 x 3 姿态 = 21 + 圆盘敌机 1 + 富岳 BOSS 1 = 23

### 技术说明

- 视角：3/4 俯视（iFighter 经典 STG 视角），非正俯视
- 螺旋桨：侧面视角，显示为薄模糊边缘线条
- roll：仅翼面明暗/透视变化，机身不旋转
- roll_left / roll_right AI 独立生成（非镜像）

### 文件变更

- 重建 `assets/bak/`，23 个 PNG
- 更新 DesignLog.md
