...
func _create_player() -> void:
	...
	_player.body_entered.connect(_on_player_body_entered)
...
func _on_hitbox_area_entered(area: Node) -> void:
	if _is_invincible or _god_mode:
		return

	# 敌弹命中
	if area is ColorRect and area.get_meta("is_player_bullet", true) == false:
		_on_player_hit(area)
